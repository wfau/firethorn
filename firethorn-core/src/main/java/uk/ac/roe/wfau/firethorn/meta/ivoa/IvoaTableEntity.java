...
@NamedQueries(
        ...
        @NamedQuery(
            ...
            query = "FROM IvoaTableEntity WHERE parent = :parent ORDER BY name desc"
            ),
        @NamedQuery(
            name  = "IvoaTable-select-parent.name",
            query = "FROM IvoaTableEntity WHERE ((parent = :parent) AND (name = :name)) ORDER BY name desc"
            ),
        @NamedQuery(
            name  = "IvoaTable-search-parent.text",
            query = "FROM IvoaTableEntity WHERE ((parent = :parent) AND (name LIKE :text)) ORDER BY name desc"
            )
        }
    )
