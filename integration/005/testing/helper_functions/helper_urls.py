    ...
    def POST (self):
        ...
        if data.generated_tap !="" and data.email !="" and data.query !="":
            try:
                ...
                if votable =="" and pathname!="":                
                    html = "<div style=\"color:red;margin-left:50px;\">Your query request was not executed. Please check your query</div>"    
                else: 
                    return_val = freeform_sql.write_format_from_json('vo',votable )
                    filename = data_tables_processing.save_as_temp_file(return_val)
                    votable_url =  publicly_visible_temp_dir + filename
