...
@author: stelios
'''
from helper_functions import fourFn
import atpy
import freeform_sql
import json
import operator
...
import web
import urlparse
from config import global_precision, publicly_visible_temp_dir
...
class data_tables_processing:
    ...
    def getRowIndexList(self, jsn_encoded_stored_data, data, sort_col, sort_order,search_rexp,_broadcast_url):
        ...
        if _broadcast_url!="" and _broadcast_url!=None:
            ...
            try:
                ...
                import urllib2
                import tempfile
                U = urllib2.urlopen(_broadcast_url)
                ...
                print >>tf,R
                tf.file.flush()
                votable = atpy.Table(tf.name,type='vo')
        ...
        for i in rows:
            ...
            for col in i:
                ...
                if type (i[col])==int or type (i[col])==float or type (i[col])==long :
                    if (getattr(data, 'col_' + col)!="null" and getattr(data, 'col_' + col)!=None):
                      
                        if abs(i[col]- float(getattr(data, 'col_' + col)))>global_precision:
                            index=-1
                    else :
                        if (str(i[col]).lower() !="null" and i[col] !=None  and str(i[col]).lower() !="nan" and (str(i[col]).strip() != str(getattr(data, 'col_' + col)).strip())):
                            index =-1
 
                else:
                    
                    if (str(i[col]).strip().lower() != str(getattr(data, 'col_' + col)).strip().lower()):
                        index=-1
    ...
    def  column_search_math_expr_multiple_points(self, values, points, cols, expressions):
        ...
        arith = fourFn.Arith( vars_ )
    
        
        if len(points)>1:
            for i in points[0]:
                if result != True:
                    if (abs(abs(arith.eval(expressions[0].lower()))-abs(float(i)))<global_precision and abs(abs(arith.eval(expressions[1].lower()))-abs(float(points[1][counter])))<global_precision ):
                        result = True
                counter = counter + 1
    ...
    def column_search_math_expr(self, values, points, cols):
        ...
        if len(points)==2:
            ...
            result = (abs(abs(arith.eval(points[0][0].lower()))-abs(float(points[0][1])))<global_precision) and (abs(abs(arith.eval(points[1][0].lower()))-abs(float(points[1][1])))<global_precision)
    ...
    def save_as_temp_file(self, table):
        ...
        try:
            ...
            return_val = publicly_visible_temp_dir + filename
    ...
    def GET(self):
        ...
        try:       
                ...
                if getRowIndexes =="true":
                    ...
                    return_val = json.dumps(return_dict)                      
                elif sort_val != "" and col_to_sort !="" and (rexp=="" or rexp==None) and sort_val != None and col_to_sort !=None:
    ...
    def POST(self):
        ...
        data = web.input(_action = "process", _broadcast_url = "", _sSearch="", _sSortDir_0 ="" ,_get_sort_col ="", getRowIndexes = "false", pathname="", rexp = "" , iSortCol_0=-1,iDisplayStart=1, iDisplayLength=10, sSortDir_0="asc", sEcho=0, sSearch=None, pfn=None, y_expr="", x_expr="", save_as = "", visibleCols = "")
        return_val = ""
        f = ''
        col_to_sort = ""
        sort_val = ""
        ...
        try:      
            ...
            if data.visibleCols !="":
                import csv
                visible_columns = csv.reader([data.visibleCols], skipinitialspace=True)
            ...
            if save_as != "" and save_as != None:
                ...
                if jsn !="" and jsn!=None:
                    ...
                    lst.append(jsn[0])
                    lst.append(return_dict['aaData'])
                    if visible_columns!=None:
                        for i in visible_columns:
                            col_data.append(i)
                    col_data = col_data[0] if len(col_data)>0 else None
                    return_val = freeform_sql.write_format_from_json(save_as,lst, col_data )
                else:
                    return_val = "No results returned"
