...
from freeform_config import survey_prefix
...
def handleXML(xml):
    ...
    res+= handleSchemas(schemas)
    return res

def handleSchemas(schemas):
    """ 
    Handle each schema element from the input list
  
    @param schemas: Handle a list of schema elements
    @return: HTML content for schemas
    """
    
    res = ""    
    for each_schema in schemas:
        alias = getText(each_schema.getElementsByTagName("name")[0])
        tables = each_schema.getElementsByTagName("table")
        res += handleTables(tables, alias)
        
    return res

def handleTables(tables, alias):
    """ 
    ...
    for each_tbl in tables:
        res += '<div class="accordion">' + handleTable(each_tbl, alias) + '</div>'
        res += "<br>"
    return res



def handleTable(table, alias):
    """
    ...
    res = ""
    res += handleTableName(table.getElementsByTagName("name")[0], alias)
...
def handleTableName(name, alias):
    """ 
    ...
    """
    return_val = '<p class="heading">' + alias + '.%s <img style="float:right;margin-top:5px" src="' + survey_prefix + '/static/static_vo_tool/asc.gif"/> </p>'
    return  return_val % getText(name)
...
def handleColumnName(name):
    ...
    @return: HTML content with the text of the node
    """
    return_val = '<a class="expand">%s</a> <img style="float:right;margin-top:5px" src="' + survey_prefix + '/static/static_vo_tool/asc.gif"/>'
    return return_val % getText(name)
