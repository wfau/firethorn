		...
		if ( oConfig.bHeader )
		{
			for ( i=0, iLen=dt.aoColumns.length ; i<iLen ; i++ )
			{
				if ( aColumnsInc[i] )
				{
					...
					var str_header = this._fnBoundData( sLoopData, oConfig.sFieldBoundary, regex );
					sData += str_header.replace('\t','').replace('\n',' ').replace('\r','') +
		...
		for ( j=0, jLen=dt.aiDisplay.length ; j<jLen ; j++ )
		{
			...
			for ( i=0, iLen=dt.aoColumns.length ; i<iLen ; i++ )
			{
				if ( aColumnsInc[i] )
				{
					...
					sLoopData = this._fnHtmlDecode( sLoopData );
					var str = this._fnBoundData( sLoopData, oConfig.sFieldBoundary, regex);
					
					/* Bound it and add it to the total data */
					
					sData += str.replace('\t','').replace('\n',' ').replace('\r','') +
					 	oConfig.sFieldSeperator;
				}
			}
			sData = sData.slice( 0, oConfig.sFieldSeperator.length*-1 );
 ...
 */
TableTools.DEFAULTS = {
	"sSwfPath":         survey_prefix + "/static/static_vo_tool/media/swf/copy_cvs_xls_pdf.swf",
	"sRowSelect":       "none",
	"fnPreRowSelect":   null,
	"fnRowSelected":    null,
