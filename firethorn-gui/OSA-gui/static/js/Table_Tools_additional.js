 ...
 * @param oData
 */
jQuery.fn.dataTableExt.oApi.fnMultiFilterStandard = function( oSettings, oData ) {
...
/**
 * Filter all existing DataTables
 */
...
function fnSaveAsGetURLforViewer (save_type, oTable, pathname, base_url, x_expr, y_expr) { 
	...
	if (x_expr!="" && y_expr!=""){
		var get_url = base_url  + '/data_tables_processing' + '?' + jQuery.param(params) + "&x_expr=" + encodeURIComponent(x_expr) + "&y_expr=" + encodeURIComponent(y_expr);
	} else {
		var get_url = base_url +  '/data_tables_processing' + '?' + jQuery.param(params);
...
getColumnsShown = function(dTable)
{
	vCols = new Array();
 ...
 * 
 */
function fnSaveAsCustom (save_type, oTable, pathname, xypair){
	...
	if (xypair!=null) {
		params.push({'name' : 'x_expr', 'value' : xypair[0]});
		params.push({'name' : 'y_expr', 'value' : xypair[1]});

	}
	columnsShown = getColumnsShown(oTable);   
	params.push({'name' : 'visibleCols', 'value' : columnsShown});
	var aoPost = params;
    ...
    nForm.setAttribute( 'action', base_url + '/data_tables_processing');
...
function fnGetSelectedEval( oTableLocal, x_axis, y_axis, cols ) {
    	console.log(oTableLocal, x_axis, y_axis, cols );
