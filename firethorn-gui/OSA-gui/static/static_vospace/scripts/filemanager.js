...
var fileConnector = vospace_fileConnector; 

var capabilities = new Array('select', 'download', 'rename', 'delete');

// Get localized messages from file 
// through culture var or from URL
if($.urlParam('langCode') != 0 && file_exists (survey_prefix + '/static/static_vospace/scripts/languages/'  + $.urlParam('langCode') + '.js')) culture = $.urlParam('langCode');
var lg = [];
$.ajax({
  url: survey_prefix + '/static/static_vospace/scripts/languages/'  + culture + '.js',
  async: false,
  dataType: 'json',
...
var renameItem = function(data){
	...
	var getNewName = function(v, m){
		...
		if(rname != ''){
			...
			var oldPath = $('#preview h1').attr('title');	
			var connectString = fileConnector + '?mode=rename&old=' + oldPath + '&new=' + givenName;
...
var getFolderInfo = function(path){
	...
	$('#fileinfo').html('<img id="activity" src="' + survey_prefix + '/static/static_vospace/images/wait30trans.gif" width="30" height="30" />');
