...
class services:
    ...
    def __generate_html_content(self, data):
        ...
        if data == [] or data== None:
            ...
            return_html = str(render.select_service_response('<a href=' + config.local_hostname['services'] + '?'+ config.get_param + '='  +  urllib2.quote(data["ident"].encode("utf8")) + '>' + data["name"] + '</a>',datetime.strptime(data["created"], "%Y-%m-%dT%H:%M:%S.%f").strftime("%d %B %Y at %H:%M:%S"), datetime.strptime(data["modified"], "%Y-%m-%dT%H:%M:%S.%f").strftime("%d %B %Y at %H:%M:%S")))
