 ...
 * </p>
 * <p>
 *	When overridding this class, you must implement {@link #writeMetadata(Object, PrintWriter, ADQLExecutor)} and
 *	{@link #writeData(Object, DBColumn[], OutputStream, ADQLExecutor)}.
 *	Both are called by {@link #writeResult(Object, OutputStream, ADQLExecutor)}. Finally you will also have to implement
 *	{@link #writeResult(Object, ADQLExecutor)}, which must format the given result into a VOTable saved in some way accessible
...
public abstract class VOTableFormat<R> implements OutputFormat<R> {
	 ...
	 *	<li><i>{@link #writeHeader(PrintWriter, ADQLExecutor)}</i></li>
	 *	<li>&lt;TABLE&gt;</li>
	 *	<li>&lt;DATA&gt;</li>
	 *	<li><i>{@link #writeData(Object, DBColumn[], OutputStream, ADQLExecutor)}</i></li>
	 *	<li>&lt;/DATA&gt;</li>
	 ...
	 * </ul>
	 * 
	 * @see tap.formatter.OutputFormat#writeResult(java.lang.Object, java.io.OutputStream, tap.ADQLExecutor)
	 */
	public final void writeResult(final R queryResult, final OutputStream output, final ADQLExecutor<R> job) throws TAPException {
		try{
			...
			if (logFormatReport)
				service.log(LogType.INFO, "Job N°"+job.getJobId()+" - Result formatted (in VOTable ; "+nbRows+" rows ; "+columns.length+" columns) in "+(System.currentTimeMillis()-start)+" ms !");
	 ...
	 * @param job			The job which generates the query result to format.
	 * 
	 ...
	 */
	protected void writeHeader(final PrintWriter output, final ADQLExecutor<R> job) throws IOException, TAPException {
		...
		output.println("\t\t<INFO name=\"QUERY\"><![CDATA["+job.getADQLQuery()+"]]></INFO>");
	}
	 ...
	 * @param job			The job which processed the result.
	 * 
	 ...
	 */
	protected abstract DBColumn[] writeMetadata(final R queryResult, final PrintWriter output, final ADQLExecutor<R> job) throws IOException, TAPException;
	 ...
	 * @throws TAPException		If there is any other error.
	 */
	protected abstract int writeData(final R queryResult, final DBColumn[] selectedColumns, final OutputStream output, final ADQLExecutor<R> job) throws IOException, TAPException;
