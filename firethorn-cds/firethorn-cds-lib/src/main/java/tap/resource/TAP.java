...
logger.debug("TAP(ServiceConnection<?>)");
		...
		if (service.uploadEnabled()){
			try {
				DBConnection<?> dbConn = service.getFactory().createDBConnection();
		...
		getUWS().setUserIdentifier(serviceConnection.getUserIdentifier());
	}
	...
	public String getCapability() {
		...
		for(OutputFormat<?> f : formats){
			...
			xml.append("\t\t<mime>").append(f.getMimeType()).append("</mime>\n");
			if (f.getShortMimeType() != null)
				xml.append("\t\t<alias>").append(f.getShortMimeType()).append("</alias>\n");
			if (f.getDescription() != null)
				xml.append("\t\t<description>").append(f.getDescription()).append("</description>\n");
			xml.append("\t</outputFormat>\n");
		}
...
logger.debug("Resource [{}]", res);
				try{
					if (res != null)
						...
						response.sendError(HttpServletResponse.SC_NOT_FOUND, "This TAP service does not have a resource named \""+resourceName+"\" !");
				}catch(TAPException tex){
					writeError(tex, response);
				}
			}
...
System.out.print("File path [" + file.getAbsolutePath() + "]");
	...
	public void writeError(TAPException ex, PrintWriter output) throws ServletException, IOException {
		...
		DateFormat dateFormat = new SimpleDateFormat(AbstractJob.DEFAULT_DATE_FORMAT);
	...
	public void updateServiceRestrictions(){
		...
		int[] retentionPeriod = service.getRetentionPeriod();
		if (retentionPeriod != null && retentionPeriod.length >= 2){
			uws.getDestructionTimeController().setDefaultDestructionInterval(retentionPeriod[0], DateField.SECOND);
			uws.getDestructionTimeController().setMaxDestructionInterval(retentionPeriod[1], DateField.SECOND);
		}

		// Execution duration:
		int[] executionDuration = service.getExecutionDuration();
		if (executionDuration != null && executionDuration.length >= 2){
			uws.getExecutionDurationController().setDefaultExecutionDuration(executionDuration[0]);
			uws.getExecutionDurationController().setMaxExecutionDuration(executionDuration[1]);
		}
	}
