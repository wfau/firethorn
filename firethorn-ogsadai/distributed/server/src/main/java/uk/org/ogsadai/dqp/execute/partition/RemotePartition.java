...
public class RemotePartition implements Partition
{
    ...
    private static final DAILogger LOG = 
        DAILogger.getLogger(RemotePartition.class);
    ...
    {
        // prepare data source on source partition
        PipelineWorkflow workflow = new PipelineWorkflow();
        
        GenericActivity create = 
            new GenericActivity("uk.org.ogsadai.CreateDataSource");
        ...
        GenericActivity deliverToRequestStatus = 
            new GenericActivity("uk.org.ogsadai.DeliverToRequestStatus");
        deliverToRequestStatus.createInput("input");
        deliverToRequestStatus.connectInput(
            "input", create.getOutput("result"));
    ...
    public void addDataSinkToSetup(String dataSink)
    {
        if (LOG.isDebugEnabled())
        {
            LOG.debug("Adding data sink " + dataSink + 
                      " to partition for : " + mNode);
        }

        // prepare data sink on source partition
        PipelineWorkflow workflow = new PipelineWorkflow();
        
        GenericActivity create = 
            new GenericActivity("uk.org.ogsadai.CreateDataSink");
        create.createInput("resourceId");
        create.createOutput("result");
    ...
    public void execute()throws ExecutionException
    {
        ...
        catch (InterruptedException e)
        {
            ...
            // try to cancel the request and exit
            try
            {
                request.destroy();
                request = null;
    ...
    public void executeSetup() throws ExecutionException
    {
        ...
        catch (RequestExecutionException e)
        {
            RequestStatus status = 
                e.getRequestResource().getLocalRequestStatus();
            if (LOG.isDebugEnabled())
            {
    ...
    public void cleanup()
    {
        ...
        for (ResourceID id : mDataSources)
        {
            DataSourceResource dataSource = 
                mNode.getDataSourceResource(id, mRequestDetails);

            try
            {
                dataSource.destroy();
                LOG.debug("Remote partition " + mNode + 
                        ": data source " + id + " destroyed.");
            }
            catch (Throwable e)
            {
                LOG.warn(e);
        ...
        for (ResourceID id : mDataSinks)
        {
            DataSinkResource dataSink = 
                mNode.getDataSinkResource(id, mRequestDetails);

            try
            {
                dataSink.destroy();
                LOG.debug("Remote partition " + mNode + 
                        ": data sink " + id + " destroyed.");
            }
            catch (Throwable e)
            {
                LOG.warn(e);
     ...
     */
    private void pollUntilRequestCompleted(RequestResource request, int pollInterval)
        throws RequestException, 
