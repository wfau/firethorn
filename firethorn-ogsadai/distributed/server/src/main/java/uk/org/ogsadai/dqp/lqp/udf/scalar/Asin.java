...
/**
 * Scalar function to calculate arcsine.
 * 
...
public class Asin extends LogicalExecutableFunctionBase {
    ...
    public Asin(Asin asin) {
        this();
        mType = asin.getParameterType();
        mResultType = asin.getOutputType();
    }
    ...
    /**
     * Puts the input parameter X and computes asin(X).
     * 
     */
    public void put(Object... parameters) {
        ...
        else
        {
            ...
            switch(mResultType) {
                case TupleTypes._DOUBLE:
                    mResult = Math.asin((Double)x);
                    break;
                case TupleTypes._BIGDECIMAL:
                    BigDecimal xb = (BigDecimal)x;
                    mResult = Math.asin(xb.doubleValue());
                    break;
            }
        }
    }
