 ...
 * </p>
 * <p>
 *	Jobs can be added thanks to {@link #update(AbstractJob)} and removed with {@link #remove(AbstractJob)}.
 ...
 *  the job knows its jobs list ({@link AbstractJob#getJobList()} != null) and its jobs list has a destruction manager.
 * </p>
 * 
...
public class DefaultDestructionManager implements DestructionManager {
	...
	protected final TreeSet<AbstractJob> jobsToDestroy;
	...
	protected AbstractJob currentJob = null;
	...
	public DefaultDestructionManager(){
		jobsToDestroy = new TreeSet<AbstractJob>(new TimeComparator());
	}

	/**
	 * <p>This method is called just after a de-serialization of the whole UWS (this destruction manager included).
	 * It reset the destruction timer.</p>
	 */
	protected synchronized void sync(){
		if (currentDate != null && timDestruction == null){
	 ...
	 * <p>
	 * 	<i><u>Note:</u>
	 * 		If this object has just been deserialized, {@link #timDestruction} is NULL.
	 * 		In this case this function resets it, obviously if it is supposed to run ({@link #currentDate} != null).
	 * 	</i>
	 * </p>
	 * 
	 * @see uws.job.manager.DestructionManager#isRunning()
	 */
	 ...
	 * <ol>
	 * 	<li><u>The destruction time of the given job is past:</u> the job is destroyed (see {@link #destroyJob(AbstractJob)}).</li>
	 ...
	 * </ol>
	 * 
	 * @see uws.job.manager.DestructionManager#update(uws.job.AbstractJob)
	 * @see #destroyJob(AbstractJob)
	 * @see #refresh()
	 */
	public synchronized void update(AbstractJob job){
	 ...
	 * 
	 * @see uws.job.manager.DestructionManager#remove(uws.job.AbstractJob)
	 * @see	#stop()
	 * @see #refresh()
	 */
	public synchronized void remove(AbstractJob job){
		if (job == null)
			return;
	...
	/* ************** */
	/**
	 * Lets a TreeSet or a TreeMap sorting {@link AbstractJob} instances in an ascending order
	 * and according to their destruction time.
	 * 
	 * @author Gr&eacute;gory Mantelet (CDS)
	 * @version 02/2011
	 */
	protected static class TimeComparator implements Serializable, Comparator<AbstractJob> {
		private static final long serialVersionUID = 1L;
