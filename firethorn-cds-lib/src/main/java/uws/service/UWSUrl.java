...
logger.debug("UWSUrl(String)");
		...
		if (baseURI.length() == 0)
			throw new UWSException(UWSException.INTERNAL_SERVER_ERROR, "[Create a UWS URL interpreter] The given base UWS URI is empty !");
	}
...
logger.debug("UWSUrl(HttpServletRequest)");
		...
		if (baseURI.length() == 0)
			throw new UWSException(UWSException.INTERNAL_SERVER_ERROR, "[Create a UWS URL interpreter] The extracted base UWS URI is empty !");
	}
...
/* ******************** */
	...
	public final UWSUrl jobName			(String jobListName, String jobId)					{ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_RUN_ID}); return url;	}
	/** Gets the UWS URL to get the <b>phase</b>. */
	public final UWSUrl jobPhase 		(String jobListName, String jobId)					{ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_PHASE}); return url;	}
	/** Gets the UWS URL to get the <b>execution duration</b>. */
	public final UWSUrl jobExecDuration (String jobListName, String jobId)					{ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_EXECUTION_DURATION}); return url;		}
	/** Gets the UWS URL to get the <b>destruction time</b>. */
	public final UWSUrl jobDestruction 	(String jobListName, String jobId)					{ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_DESTRUCTION_TIME}); return url;			}
	/** Gets the UWS URL to get the <b>error summary</b>. */
	public final UWSUrl jobError 		(String jobListName, String jobId)					{ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_ERROR_SUMMARY}); return url;			}
	/** Gets the UWS URL to get the <b>quote</b>. */
	public final UWSUrl jobQuote 		(String jobListName, String jobId)					{ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_QUOTE}); return url; 	}
	/** Gets the UWS URL to get the <b>results</b>. */
	public final UWSUrl jobResults 		(String jobListName, String jobId)					{ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_RESULTS}); return url;	}
	/** Gets the UWS URL to get the <b>specified result</b>. */
	public final UWSUrl jobResult		(String jobListName, String jobId, String resultId)	{ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_RESULTS, resultId}); return url;		}
	/** Gets the UWS URL to get the <b>parameters</b>. */
	public final UWSUrl jobParameters 	(String jobListName, String jobId)					{ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_PARAMETERS}); return url;		 		}
	/** Gets the UWS URL to get the <b>parameters/parameter</b>. */
	public final UWSUrl jobParameter 	(String jobListName, String jobId, String paramName){ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_PARAMETERS, paramName}); return url; 	}
	/** Gets the UWS URL to get the <b>owner ID</b>. */
	public final UWSUrl jobOwner		(String jobListName, String jobId)					{ UWSUrl url = jobSummary(jobListName, jobId); url.setAttributes(new String[]{AbstractJob.PARAM_OWNER}); return url;	}
...
/* ******************************* */
	...
	public final String deleteJob				(String jobListName, String jobId)										{ return jobSummary(jobListName, jobId)		+"?"+AbstractJob.PARAM_ACTION+"="+AbstractJob.ACTION_DELETE; }
	/** Gets the UWS URL <b>(HTTP-GET ONLY)</b> to <b>start</b> the specified job. */
	public final String startJob				(String jobListName, String jobId)										{ return jobPhase(jobListName, jobId)			+"?"+AbstractJob.PARAM_PHASE.toUpperCase()+"="+AbstractJob.PHASE_RUN; }
	/** Gets the UWS URL <b>(HTTP-GET ONLY)</b> to <b>abort</b> the specified job. */
	public final String abortJob				(String jobListName, String jobId)										{ return jobPhase(jobListName, jobId)			+"?"+AbstractJob.PARAM_PHASE.toUpperCase()+"="+AbstractJob.PHASE_ABORT; }

	/** Gets the UWS URL <b>(HTTP-GET ONLY)</b> to change the run ID. */
	public final String changeJobName			(String jobListName, String jobId, String newName)						{ return jobName(jobListName, jobId)			+"?"+AbstractJob.PARAM_RUN_ID.toUpperCase()+"="+newName; }
	/** Gets the UWS URL <b>(HTTP-GET ONLY)</b> to change the destruction time. */
	public final String changeDestructionTime	(String jobListName, String jobId, String newDestructionTime)			{ return jobDestruction(jobListName, jobId)	+"?"+AbstractJob.PARAM_DESTRUCTION_TIME.toUpperCase()+"="+newDestructionTime; }
	/** Gets the UWS URL <b>(HTTP-GET ONLY)</b> to change the execution duration. */
	public final String changeExecDuration		(String jobListName, String jobId, String newExecDuration)				{ return jobExecDuration(jobListName, jobId)	+"?"+AbstractJob.PARAM_EXECUTION_DURATION.toUpperCase()+"="+newExecDuration; }
